...
fun typeof(LITERAL(v), Delta, Gamma) =
  (case LITERAL(v)
    ...
    | LITERAL(PAIR(v, NIL)) => listtype (typeof(LITERAL(v), Delta, Gamma))
    | LITERAL(PAIR(v, v'))  => 
        let 
          val v1 = typeof(LITERAL(v), Delta, Gamma)
          val v2 = typeof(LITERAL(v'), Delta, Gamma)
        in 
          if eqType(listtype v1, v2)
          then v2
  ...
  | typeof(SET (x, e), Delta, Gamma) = 
          ...
          in 
            if eqType(tau_x, tau_e) 
            then tau_x 
            else raise TypeError "Mismatched types in set expression"
          end
